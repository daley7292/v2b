...
class TelegramService {
    ...
    public
    function sendMessage(int $chatId, string $text, string $parseMode = '') {
    ...
    public
    function approveChatJoinRequest(int $chatId, int $userId) {
    ...
    public
    function declineChatJoinRequest(int $chatId, int $userId) {
    ...
    public
    function sendMessageWithAdmin($message, $isStaff = false) {
    ...
    private
    function discoverCommands(string $directory): array {
        $commands = [];
        ...
        foreach(glob($directory.
            ...
            if (!class_exists($className)) {
                require_once $file; // 避免 class not found
            }
            ...
            try {
                ...
                if (
                    ...
                    // 读取属性值，不需要实例化
                    $command = $commandProp - > isStatic() ? $commandProp - > getValue() : $ref - > newInstanceWithoutConstructor() - > command;

                    $description = $descProp - > isStatic() ? $descProp - > getValue() : $ref - > newInstanceWithoutConstructor() - > description;
